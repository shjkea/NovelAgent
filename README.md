...
NovelAgent 不是一次设计完成的演示项目，而是在真实长篇生成中逐步重构出来的。四个阶段分别解决了不同层级的问题：

| 阶段 | 实际运行与主要变化 | 暴露的问题与后续方向 |
| --- | --- | --- |
| **早期章节流水线** | 完成过一次约 **1000 章的连续生成运行**，验证了 Plan、Draft、Review、Revision、Summary 和 Memory 串联生产长篇的基本可行性。 | 长跑中逐渐出现记忆污染、状态漂移、上下文膨胀、连续性错误累积和中断恢复不足。单章能够继续生成，不代表整部长篇仍然保持一致。 |
| **长篇能力扩展** | 在原有流水线上加入任务卡、按章节裁剪大纲、历史状态重建、结构化 Review、跨章审计、批量修复、回滚和 Canon/DLC 隔离，并完成另一轮 **100+ 章实际运行**。 | 功能明显增强，但正文和派生数据仍以多个步骤分别写入；审查主要关注宏观剧情与跨章连续性，最后一轮修订也可能未经完整复审。 |
| **第一次架构重构** | 围绕阶段合同、Plan 检查、最终质量门和更严格的章节承接重新组织生成流程，试图同时解决剧情偏航与连续章节反复套用“日期开场—完整作息—夜晚收尾”的日记式结构。 | 实跑发现新的架构盲点：阶段合同可能静默降级；Plan 会服从大纲却不验证行动是否可行、数据是否充分；大纲目标可能压过因果合理性；同源 Review 容易连续假阴性；章内数字、时间、物品、动作和证据—结论缺少独立检查；弱审查不报错时，强审查没有纠正机会。过多提示词约束还会让模型机械证明自己“遵守规则”，使正文趋于公式化。 |
| **当前事务化架构** | 增加合同条目完整性校验与重试、统一的 Plan 评分语义、事件式章节切点、日记骨架检测、章内事实与论证检查、默认强模型 Review、逐轮复审、候选隔离、结构化 Handoff、Canon 账本、事务提交和中断恢复，并再次完成 **100+ 章连续生成测试**。 | 当前重点已经从“能否连续生成”转向“如何在不牺牲人物声音和小说自然度的前提下维持长期一致性”。自动审查仍可能误判或漏判，因此作者继续保留最终提交权。 |

这些数字表示系统确实完成过相应规模的实际生产运行，并不表示所有章节均无错误、无需人工监督，也不构成标准化模型质量 benchmark。
...
> **复杂专业场景需要细纲。** 如果章节涉及电竞比赛、体育赛事、战术对抗、推理流程或其他具有明确规则与状态变化的内容，不要只写“某队赢下比赛”这类方向性目标。应在 Outline 中预先锁定赛制、双方名单、比分、地图顺序、Ban/Pick、使用英雄或阵容、换人、关键回合及最终结果。NovelAgent 更适合根据明确约束把过程写成正文，不应被期待在生成时同时发明一套长期自洽的专业规则和比赛细节。
...
- 电竞比赛、体育赛事、战术对抗等规则密集内容高度依赖细化大纲；若只给方向性目标，模型可能自行补造赛制、比分、地图、阵容或关键过程，并在后续章节产生矛盾。
...
- `story/outline.md`：章节大纲；规则密集的复杂场景应同时锁定比分、地图、阵容、Ban/Pick、关键过程和最终结果
...
DLC 是与正式 Canon 隔离的可选扩写通道，可使用本地模型或 Grok 补充主写作模型受限或不适合处理的场景。DLC 内容不会自动进入 Summary、Memory 或当前状态，是否采用由作者单独决定。
